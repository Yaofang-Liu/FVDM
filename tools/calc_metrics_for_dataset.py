...
def subprocess_fn(rank, args, temp_dir):
    ...
    for metric in args.metrics:
        ...
        new_result_dict = {'path': args.gen_dataset_kwargs['path']}
        # Update the new dictionary with the existing result_dict
        new_result_dict.update(result_dict)
        # Replace the original result_dict with the new one
...
def calc_metrics_for_dataset(ctx, metrics, real_data_path, fake_data_path, mirror, resolution, subsample_factor, gpus, verbose, use_cache: bool, num_runs: int):
    dnnlib.util.Logger(should_flush=True)
    ...
    # Initialize dataset options for fake data. subsample_factor for fake data is 1, because model already trained with interval
    args.gen_dataset_kwargs = dnnlib.EasyDict(
        ...
        subsample_factor = 1,
        use_labels=False,
        # discard_short_videos = True # Important for 128 frame eval
    )
...
@click.option('--subsample_factor', help='Sampling factor, i.e. decreasing the temporal resolution', type=int,default=1, metavar='INT')
