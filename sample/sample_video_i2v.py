...
def main(args):
    ...
    for ii in range(eval_num):
        ...
        if using_gt:
            ...
            for step, video_data in enumerate(loader):
                if kk == 2048:
                    ...
                    for i, chunk in enumerate(video_chunks):
                        ...
                        if using_cfg:
                            ...
                            # ipdb.set_trace()
                            y = torch.tensor([int(video_name)] * 1, device=device)
                ...
                if args.dataset == 'ucf101':
                    name0 =  str(video_name[0].item()) + '_' +timestamp
                    video_name = []
                    video_name.append(name0)
                # ipdb.set_trace()
                if using_gt:
                    frame_save_path = os.path.join(args.save_video_path, video_name[0])
                else:
                ...
                ##### Save GT as frames
                frame_save_path = os.path.join(save_video_path_gt, video_name[0])
                if not os.path.exists(frame_save_path):
                    os.makedirs(frame_save_path)
                print(f'Saving frames to: {frame_save_path}')
                # Save each frame individually
        ...
        if arg.dataset == 'ucf101':
            video_name =  str(video_name) + '_' +timestamp
        if using_gt:
            frame_save_path = os.path.join(args.save_video_path, video_name)
        else:
            frame_save_path = os.path.join(args.save_video_path, 'sample' + '_' + args.ckpt.split('/')[-1][:-3] + '_' + timestamp + str(ii))
        if not os.path.exists(frame_save_path):
            os.makedirs(frame_save_path)
