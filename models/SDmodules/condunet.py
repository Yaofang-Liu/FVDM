...
class CondUNet(UNetModel):
    """
    A UNetModel that conditions on images with a pretrained img encoder in CLIP.
    ...
    def forward(
        self, x, timesteps, img_feat=None, img=None
    ):
        bsz = x.shape[0]
        hs = []
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))

        xf_out = self.text_seq_proj(img)
        xf_out = xf_out.permute(0, 2, 1)
        emb = emb + self.text_feat_proj(img_feat)
#         xf_out = th.cat(
#             [
#                 self.clip_tok_proj(y).reshape(bsz, -1, self.clip_emb_mult),
#                 xf_out,
